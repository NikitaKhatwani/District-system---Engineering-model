...
class TES(BaseModel):
    ...
    # @computed_field(return_type=MySeries)   
    # @property
    def hotThermocline(self,hotThafterLoss_supply,hotThafterLoss_return):
        result = self.HW_TankVol-hotThafterLoss_supply-hotThafterLoss_return
        return result
    ...
    def hotLoadShiftChargeRate(self):
        mask_OG =  pd.Series((self.hotStatus == "Discharge") | (self.hotStatus == "Hold"))
        ...
        valid_values = [value1, value2, value3, value4][0]
    ...
    def TES_H_flow(self):
        ...
        return (self.totalHeating_load/500/(self.H_tempTES-self.districtHWRT).astype(float))[0]
    ...
    def H_flowOutTES(self):
        print("hhhh",self.TES_H_flow)
        minimum =min(self.TES_H_flow,self.hotMaxflow,(self.hotThafterLoss_supply/60).astype(float))
    ...
    # @computed_field(return_type=MySeries)   
    # @property
    # def TES_H_rerun(self):

    # @computed_field(return_type=MySeries)   
    # @property
    def TES_H_calculate(self):

            first_hotCapacityHr=self.hotCapacityHr
            first_hotThafterLoss_supply = self.hotThafterLoss_supply
            first_hotThafterLoss_return = self.hotThafterLoss_return
            first_hotThermocline = self.hotThermocline(first_hotThafterLoss_supply,first_hotThafterLoss_return)

            # first_ = self.
            # first_ = self.

            df3 = pd.DataFrame({
            "TES Hot Capacity hr-1 (%)": first_hotCapacityHr,
            "TES Hot Th after losses (Gal)_supply temp": first_hotThafterLoss_supply,
            "TES Hot Tc after losses (Gal)_return temp":first_hotThafterLoss_return,
            "TES Hot Thermocline (Gal)": first_hotThermocline,
            # "TES Cold Th after losses (Gal)_supply temp": self.coldThafterLoss_supply,
            # "TES Cold Tc after losses (Gal)_return temp":self.coldThafterLoss_return,
            # "TES Cold Thermocline (Gal)": self.coldThermocline,
            })
